...
//        Get the API user
        ...
        foreach ($sections as $section) {
            ...
            try {
                $student_enrollment = $schoology->apiResult(sprintf('sections/%s/enrollments?uid=%s', $section->id, $this->enrollment->uid))->enrollment[0];
            } catch (\Exception $e){ }
            // Check if this is an active enrollment
            if ($student_enrollment->status == 1) {
                // Count this section
                $total_sections++;
                $admin_enrollments = $schoology->apiResult(sprintf('sections/%s/enrollments?uid=%s', $section->id, $api_user->uid));
                if (count($admin_enrollments->enrollment)) {
                    // The API user is enrolled in the course.
                    if ($admin_enrollments->enrollment[0]->admin != 1 || $admin_enrollments->enrollment[0]->status > 1) {
                        // Observer is enrolled, but not an admin or inactive
                        $body = ['uid' => $api_user->uid, 'admin' => 1, 'status' => 1];
                        try {
                            $schoology->apiResult(sprintf('sections/%s/enrollments/%s', $section->id,$admin_enrollments->enrollment[0]->id), 'PUT', $body);
                        } catch (\Exception $e){
                        }
                    }
                } else {
